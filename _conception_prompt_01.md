...
Хочется в v2 избегать рассогласованностей, особенно в ключах, поэтому предлагается собирать v2-пути (`year/month/day/hour`) на основе `observation.end_time` 
...
- После успешной регистрации файла через `POST .../files` повторный запрос presigned URL для того же `s3_key` отклоняется (`403`).
...
- `demoddata`/`image` — уникальность по `s3_key` на уровне БД (constraint), повторная регистрация того же ключа — `already_registered` (не ошибка), другого — обычная новая запись.
- `waterfall`/`audio` — единственное поле на самой записи наблюдения (не отдельная таблица). Совпадающий `s3_key` — молчаливый no-op (`already_registered`). Отличающийся от уже сохранённого — отклоняется (`rejected`), перезапись через API не допускается; исправление — только через админку.

Ответ API — по каждому файлу из списка отдельно (`registered` / `already_registered` / `rejected`), чтобы станция могла безопасно повторить неполный batch после обрыва связи, не потеряв уже принятые файлы.
