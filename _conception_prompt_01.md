...
В системе предлагается расширенная статусная модель для точного отслеживания жизненного цикла наблюдения на стороне сервера и распределенных станций.
...
- **Инициация**: Генерация пачки наблюдений происходит только по явному запросу к API:
    * `POST /api/v2/stations/{id}/observations` — планирование для конкретной станции.
...
101. Настройка TTL: Время жизни сообщения в очереди RabbitMQ должно рассчитываться динамически для каждого пролета спутника (Tend−Tstart+buffer) / связанного с ним наблюдения.

102. Именование очередей: Для каждой станции удобно использовать паттерн station.{id}.tasks, чтобы DLX мог возвращать метаданные о том, откуда "прилетело" протухшее сообщение. DLX должен получать `routing key` со `station_id` через заголовки (x-dead-letter-routing-key)
