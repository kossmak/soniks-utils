...
Право станции на регистрацию файлов по наблюдению подтверждается на стороне API через `ObservationORM.station_id` - чтобы скомпрометированная станция не могла абьюзить API редактированием / порчей чужих наблюдений, сервис проверяет соответствие `observation_id`, `station_id` и `station_api_token`. 
