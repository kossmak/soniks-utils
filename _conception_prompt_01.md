...
1. Очередь RabbitMQ может работать только как транспорт сообщений - не следит за сроком жизни и актуализацией наблюдения в БД.
2. Станция отправляет подтверждение о прочтении сообщения из RabbitMQ на эндпойнти API, который обновляет статус наблюдения в БД.
3. Непрочитанные из очереди сообщения лежат пока кто-то их из неё не вычитает.
4. Отдельный процесс (в отд. контейнере) RabbitTaskMonitor проходит циклом все очереди и вычитывает без ack первое сообщение из каждой очереди.
    - если сообщение протухло, забирает его из очереди. 
    - повторяет итерацию для следующего сообщения. 
    - если очередное сообщение не протухло, то переходит к следующей очереди.
5. Отдельный процесс (в отд. контейнере) DBTaskMonitor проходит каждые 5 мин циклом все записи observations со окончанием за последние 10 мин и :
    - если статус FUTURE или станция не прислала телеметрию и не обновила статус наблюдения, дёргаем taskiq для актуализации статуса в БД 
...
107. Создать rabbit-пользователей по числу зарегистрированных станций
  - как-то доставлять пароль клиенту станции
  - завести API ? для инициации rabbit-клиента с доставкой нового пароля
