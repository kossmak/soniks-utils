...
Дата в пути (`year/month/day/hour`) берётся из `observation.start_time` — не из времени фактической заливки файла. Это позволяет корректно группировать файлы даже при отложенной заливке после восстановления связи.

> **Примечание:** дубликаты с одинаковым именем не допускаются. Имя файла формируется на стороне клиента (клиент знает `observation_id`, `start_time`, timestamp кадра).
